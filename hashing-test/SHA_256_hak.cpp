...
// {
// 	// cout << "input a string " << '\n';
// 	string message = "194351981179999999999999999999999999370841221962225232188128242134024813329861757310135304468981138209137241184117166181282101522245711521717617531911801711711903436472281721552224716315715012118" ;
// 	// cin >> message;
//  mining(message);
//
//
...
long int finalNumber(string message){
	string hash = returnHash(message);
	long int inDec = returnNumber(hash);
	return inDec;
}
...
long int returnNumber(string hash){
	...
	string first64bits = hash.substr(0,16);

	char hexd[17] ;


	strncpy(hexd, first64bits.c_str(), sizeof(hexd));
	hexd[sizeof(hexd) - 1] = 0;

	char *p;
	long int intNumber = strtol(hexd, &p, 16);
	return intNumber;
